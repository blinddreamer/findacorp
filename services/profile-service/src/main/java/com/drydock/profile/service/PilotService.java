...
import com.drydock.profile.domain.*;
import com.drydock.profile.dto.PilotProfileResponse;
import com.drydock.profile.dto.UpdatePilotRequest;
    ...
    public void upsertEnrichment(PilotEnrichedEvent event) {
        ...
        if (event.skills() != null && !event.skills().isEmpty()) {
            List<String> incomingNames = event.skills().stream()
                .map(com.drydock.common.dto.SkillDto::skillName).toList();
            ...
            for (com.drydock.common.dto.SkillDto s : event.skills()) {
